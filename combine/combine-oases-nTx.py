...
for filename in os.listdir('.'):
    ...
    for line in f:
        if line.startswith('>'):
            seq_h = line.strip().replace('_Transcript', '')
            seq_h = seq_h.replace('Locus', sample_name)
            seq_h = seq_h.replace('_Confidence', '')
            seq_h = seq_h.replace('_Length', '')
            seq_list[seq_h] = []
        else:
