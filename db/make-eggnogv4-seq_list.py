...
for filename in os.listdir(dirname_members):
    ...
    sys.stderr.write('Read %s\n' % filename_members)
    f = gzip.open(filename_members, 'rt')
    for line in f:
        if line.startswith('#'):
            continue
        tokens = line.strip().split('\t')
        tmp_grp = tokens[0]
        if tmp_grp not in members:
            members[tmp_grp] = []
        tmp_id = tokens[1]
...
f_out.write('#NOG_ID\tCountMembers\tLongestID\t' +
            'Q50_ID\tLongestLen\tQ25_len\tQ50_len\tQ75_len\n')
for tmp_grp in sorted(members.keys()):
    ...
    max_seq_id = ';'.join([x for x in members[tmp_grp]
                           if seq_len[x] == max_seq_len])
    q50_seq_len = seq_len[seq_id_list[int(len(seq_id_list)*0.5)]]
    q50_seq_id = ';'.join([x for x in members[tmp_grp]
                           if seq_len[x] == q50_seq_len])
    ...
    f_out.write('%s\t%d\t%s\t%s\t%d\t%d\t%d\t%d\n' %
                (tmp_grp, len(members[tmp_grp]), max_seq_id,
                 q50_seq_id, max_seq_len,
                 q25_seq_len, q50_seq_len, q75_seq_len))
f_out.close()
