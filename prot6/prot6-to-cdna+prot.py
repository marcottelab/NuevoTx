...
for tmp_filename in os.listdir('.'):
    ...
    ## For BLASTP output
    if tmp_filename.find('.bp+_tbl') >= 0 and tmp_filename.find('treefam9') >= 0:
        sys.stderr.write('Read %s\n' % tmp_filename)
        ...
        for line in f_bp:
            ...
            if tmp_frame not in blastp_list[seq_id]:
                blastp_list[seq_id][tmp_frame] = {'targets':[], 'family':[]}
                blastp_list[seq_id][tmp_frame]['bits'] = bits
            elif blastp_list[seq_id][tmp_frame]['bits'] < bits:
                blastp_list[seq_id][tmp_frame]['bits'] = bits

            blastp_list[seq_id][tmp_frame]['targets'].append(t_id)
            blastp_list[seq_id][tmp_frame]['family'].append(family_id)
        f_bp.close()
        sys.stderr.write('Total Sequences: %d\n' % len(blastp_list))
...
for tmp_h in cdna_seq_list.keys():
    ...
    if tmp_h not in blastp_list:
        if tmp_h not in hmmer_list:
            ...
            #print(tmp_h, hmmer_list[tmp_h])
            seq_type = 'orphan'
            best_frame = list(hmmer_list[tmp_h].keys())[0]
    else:
        if len(blastp_list[tmp_h]) == 1:
            best_frame = list(blastp_list[tmp_h].keys())[0]
            best_target_list = list(set(blastp_list[tmp_h][best_frame]['targets']))

            if len(best_target_list) >= 5:
                seq_type = 'coding'
            else:
                seq_type = 'orphan'
        else:
            ...
            for tmp_frame in blastp_list[tmp_h].keys():
                target_list = list(set(blastp_list[tmp_h][tmp_frame]['targets']))
                tmp_target_count = len(target_list)
                frame2count[tmp_frame] = tmp_target_count
                frame2bits[tmp_frame] = blastp_list[tmp_h][tmp_frame]['bits']
            
            sorted_frame = sorted(frame2bits.keys(), key=frame2bits.get, reverse=True)
    ...
    if seq_type == 'noncoding':
        ...
        if seq_type == 'coding':
            tmp_best_bits = blastp_list[tmp_h][best_frame]['bits']
            f_prot_coding.write('>p.%s\n%s\n' % (seq_h, tmp_prot_seq))
            f_cdna_coding.write('>t.%s\n%s\n' % (seq_h, tmp_cdna_seq))
            f_prot_targets.write('p.%s\t%.1f\t%s\n' % (seq_h, tmp_best_bits, ','.join(sorted(best_target_list))))
