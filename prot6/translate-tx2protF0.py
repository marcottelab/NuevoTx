...
for line in f_fa:
    if( line.startswith('>') ):
        ...
        seq_list[seq_h].append(line.strip().replace(' ','').upper())
f_fa.close()
...
for tmp_h in sorted(seq_list.keys()):
    ...
    #for tmp_pf in tmp_p6.keys():
        ...
        tmp_pos = tmp_p.find(longest_pep) 
        sys.stderr.write('%s|%s: %d - %d (cdna:%d - %d)\n'%(tmp_h, tmp_pf, tmp_pos, tmp_pos+len(longest_pep), tmp_pos*3, (tmp_pos+len(longest_pep))*3))
        longest_cdna = tmp_nseq[0:][tmp_pos*3:(tmp_pos+len(longest_pep)+1)*3]
        print('>%s|%s\n%s'%(tmp_h,tmp_pf,longest_pep))
        print('>%s|%s\n%s'%(tmp_h,tmp_pf,longest_cdna))
f_pfa.close()
